...
namespace DuiLib
{
	...
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ )
	{
		...
		pDescription	= new CLabelUI();
		pEndOption		= new CCheckBoxUI();
	...
	bool CTreeNodeUI::AddAt( CControlUI* pControl, int iIndex )
	{
		...
		if(pTreeView && pIndexNode)
			return pTreeView->AddAt((CTreeNodeUI*)pControl, iIndex);
		else 
	...
	bool CTreeViewUI::SetItemCheckBox( bool _Selected,CTreeNodeUI* _TreeNode /*= NULL*/ )
	{
		if(_TreeNode)
		{
			if(_TreeNode->GetCountChild() > 0)
			{
				...
				//change 2016.1.8
				if((_TreeNode->nSelCnt != _TreeNode->mTreeNodes.m_nCount)
					&&(_TreeNode->nSelCnt  > 1))
				{
					if(_Selected)
					{
					...
					{
						_TreeNode->nSelCnt = _TreeNode->mTreeNodes.m_nCount;
					}
				}
				for(int nIndex = 0;nIndex < nCount;nIndex++)
				{
