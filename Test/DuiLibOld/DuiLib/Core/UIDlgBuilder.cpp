    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 9:
				...
				else if( _tcsicmp(pstrClass, DUI_CTR_ANIBUTTON) == 0 )        pControl = new CAniButtonUI;
                break;
            case 10:
				...
				else if( _tcsicmp(pstrClass, DUI_CTR_ANIPICTURE) == 0 )       pControl = new CAniPictureUI;
                break;
			case 11:
				if (_tcsicmp(pstrClass, DUI_CTR_CHILDLAYOUT) == 0)			  pControl = new CChildLayoutUI;
				break;
			case 13:
				if (_tcsicmp(pstrClass, DUI_CTR_HOTSHOWLAYOUT) == 0)		  pControl = new CHotShowLayoutUI;
				break;
            case 14:
