...
LRESULT WindowImplBase::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if( pt.x >= rcClient.left + rcCaption.left && pt.x < rcClient.right - rcCaption.right \
		&& pt.y >= rcCaption.top && pt.y < rcCaption.bottom ) {
			...
			if( pControl &&
				pControl->GetInterface(DUI_CTR_BUTTON) == NULL && 
				pControl->GetInterface(DUI_CTR_OPTION) == NULL &&
...
LRESULT WindowImplBase::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if( !::IsIconic(*this) && 
		(szRoundCorner.cx != 0 || szRoundCorner.cy != 0 			
			|| m_PaintManager.GetShowShadow())	// Mark by linzw--阴影类处理
		) 
	{
		CDuiRect rcWnd;
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	// Mark by linzw--阴影类处理
	if (m_PaintManager.GetShowShadow())
	{
		m_WndShadow.Create(m_hWnd);
		CDuiString sImage = m_PaintManager.GetShadowImage();
		if (sImage.IsEmpty())
		{
			DWORD dwSize = m_PaintManager.GetShadowSize();
			DWORD dwSharpness = m_PaintManager.GetShadowSharpness();
			DWORD dwDarkness = m_PaintManager.GetShadowDarkness();
			COLORREF dwColor = m_PaintManager.GetShadowColor();
			RECT rcPos = m_PaintManager.GetShadowPos();

			m_WndShadow.SetSize(dwSize);
			m_WndShadow.SetSharpness(dwSharpness);
			m_WndShadow.SetDarkness(dwDarkness);
			m_WndShadow.SetColor(dwColor);
			m_WndShadow.SetPosition(rcPos.left, rcPos.top);
		}
		else
		{
			RECT rcCorner = m_PaintManager.GetShadowCorner();
			RECT rcHole = CDuiRect();
			m_WndShadow.SetImage(sImage.GetData(), rcCorner, rcHole);			
		}
	}
	// End by linzw

	// by sundh
	// http://192.168.2.10/zentao/bug-view-368.html
	// 自己先调用一次，防止之后窗口自动切回来
