...
namespace DuiLib {
		...
		DWORD dwAlignment/* = eMenuAlignment_Left | eMenuAlignment_Top*/)
	{
		...
		HWND hwnd = (m_pOwner == NULL) ? pMainPaintManager->GetPaintWindow() : m_pOwner->GetManager()->GetPaintWindow();

		Create(hwnd, NULL, WS_POPUP , WS_EX_TOOLWINDOW | WS_EX_TOPMOST, CDuiRect());
...
// 		m_pm.GetShadow()->Create(&m_pm);
	...
	void CMenuElementUI::DrawItemIcon(HDC hDC, const RECT& rcItem)
	{
		if (!m_strIcon.IsEmpty() && !(m_bCheckItem && !GetChecked()))
		{
			...
			pStrImage.Format(_T("file='%s' dest='%d,%d,%d,%d'"), 
				m_strIcon.GetData(),
	...
	void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT& rcItem)
	{
		if (m_bShowExplandIcon)
		{
			...
			//m_cxyFixed.cx = GetManager()->GetDPIObj()->Scale(m_cxyFixed.cx);
			//m_cxyFixed.cy = GetManager()->GetDPIObj()->Scale(m_cxyFixed.cy);
			int padding = 0;// GetManager()->GetDPIObj()->Scale(ITEM_DEFAULT_EXPLAND_ICON_WIDTH) / 3;
...
// 			const TImageInfo *pImageInfo = GetManager()->GetImageEx(pDrawInfo->sImageName, NULL, 0);
			const TImageInfo *pImageInfo = GetManager()->GetImageEx(strExplandIcon, NULL, 0);
	...
	SIZE CMenuElementUI::EstimateSize(SIZE szAvailable)
	{
		...
		if(cXY.cy == 0) {
			...
			if( pInfo->bShowHtml ) {   
				int nLinks = 0;
				CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, sText, iTextColor,\
					NULL, NULL, nLinks, pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle);
			}
			else {
				CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, sText, iTextColor,\
					pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle);
			}
		...
		CMenuElementUI::m_cxyFixed.cy = cXY.cy;//MulDiv(cXY.cy, 100, GetManager()->GetDPIObj()->GetScale());
		CMenuElementUI::m_cxyFixed.cx = cXY.cx;//MulDiv(cXY.cx, 100, GetManager()->GetDPIObj()->GetScale());
		return cXY;
	}
	...
	void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcsicmp(pstrName, _T("checktype")) == 0 ) {		
