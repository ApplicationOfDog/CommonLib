...
class CComboWnd : public CWindowWnd,INotifyUI
{
public:
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE )
	{
        ...
        m_pm.Init(m_hWnd);
		LPCTSTR defScrollBar = m_pOwner->GetManager()->GetDefaultAttributeList(_T("VScrollBar"));
		if(defScrollBar!=NULL && *defScrollBar!=0)
			m_pm.AddDefaultAttributeList(_T("VScrollBar"),defScrollBar);
		defScrollBar = m_pOwner->GetManager()->GetDefaultAttributeList(_T("HScrollBar"));
		if(defScrollBar!=NULL && *defScrollBar!=0)
			m_pm.AddDefaultAttributeList(_T("HScrollBar"),defScrollBar);
		...
		///<remark by="Aweigh" date="2017/3/23">
		/// 为解决combox子项打开时无法直接定位到选择项问题(默认都是从第一项开始显示)
		/// 把CComboWnd添加到消息队列
		///</remark>
