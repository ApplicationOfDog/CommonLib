...
public:
	...
	virtual void InitCheckInfo()
	{
		if (!m_pMenuCheckInfo)
		{
			m_pMenuCheckInfo = new std::map<CDuiString, bool>();
		}
	}

	virtual void UninCheckInfo()
	{
		if (m_pMenuCheckInfo)
		{
			m_pMenuCheckInfo->clear();
			m_pMenuCheckInfo = NULL;
		}
	}

	// 注意：由于包含map限dll内部使用，故实现方法采用InitCheckInfo和UninCheckInfo
