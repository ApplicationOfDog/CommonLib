...
namespace DuiLib
{
	...
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ )
	{
		...
		if(_ParentNode)
		{
			...
			pDottedLine->SetFixedWidth(_ParentNode->GetDottedLine()->GetFixedWidth()+m_uItemIndent);
	...
	bool CTreeNodeUI::AddChildNode( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		if(pTreeView){
			...
			if(!pNode || !pNode->GetLastNode())
				nRet = pTreeView->AddAt(_pTreeNodeUI,GetTreeIndex()) >= 0;
			else nRet = pTreeView->AddAt(_pTreeNodeUI,pNode->GetLastNode()->GetTreeIndex()) >= 0;
		}
	...
	void CTreeNodeUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		if(_tcscmp(pstrName, _T("text")) == 0 )
			pItemButton->SetText(pstrValue);
		else if(_tcscmp(pstrName,_T("indent")) == 0)
			SetItemIndent(_ttoi(pstrValue));
	...
	CTreeNodeUI* CTreeNodeUI::CalLocation( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		_pTreeNodeUI->GetDottedLine()->SetFixedWidth(pDottedLine->GetFixedWidth()+m_uItemIndent);
	...
	void CTreeNodeUI::SetItemIndent( UINT _uItemIndent )
	{
		m_uItemIndent = _uItemIndent;
	}

	UINT CTreeNodeUI::GetItemIndent() const
	{
		return m_uItemIndent;
	}
    ...
    long CTreeViewUI::AddAt(CTreeNodeUI* pControl, int iIndex)
    {
        ...
        CListUI::AddAt(pTreeNode,iIndex+1);
