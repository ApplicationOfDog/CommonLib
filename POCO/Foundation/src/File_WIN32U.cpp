...
static std::string sreplace(const std::string target,const std::string & before, const std::string & after)
{
	std::string tmp(target);
	std::string before2(before);
	//Searches on tmp + before2 rather than this + before
	std::string::size_type pos = 0;
	while ((pos = tmp.find(before2, pos)) != std::string::npos) 
	{
...
std::string FileImpl::getFileNameImpl() const
{
