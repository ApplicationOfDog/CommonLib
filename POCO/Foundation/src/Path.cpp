...
std::string Path::getApplicationDirPath()
{
	char moduleName[MAX_PATH];
	GetModuleFileNameA(NULL, moduleName, sizeof(moduleName));

	std::string fpth(moduleName);
	std::string::size_type pos = fpth.rfind("\\");
	if (!(pos == std::string::npos || pos == fpth.length() - 1)) 
	{
		fpth = fpth.substr(0, pos);
	} 
	fpth += "\\";
