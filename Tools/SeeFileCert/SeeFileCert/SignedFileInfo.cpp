...
//打印证书信息
BOOL PrintCertificateInfo(PCCERT_CONTEXT pCertContext, SignerCertificate &SignerInfo);
//获取签名信息的时间戳
BOOL GetTimeStampSignerInfo(PCMSG_SIGNER_INFO pSignerInfo,
	PCMSG_SIGNER_INFO *pCounterSignerInfo);

#ifdef TEST_DEMO_GetSignatureInfo
int GetSignatureInfo(std::wstring path, SignerCertificate &SignerInfo);
int _tmain(int argc, TCHAR *argv[])
{
	SignerCertificate info;
	GetSignatureInfo(L"C:\\Program Files (x86)\\Tencent\\QQ\\Bin\\QQScLauncher.exe", info);
	return 0;
}
#endif

BOOL GetSignatureInfo(std::wstring path, SignerCertificate &SignerInfo)
{
	...
	try
	{
		lstrcpynW(szFileName, path.c_str(), MAX_PATH);
