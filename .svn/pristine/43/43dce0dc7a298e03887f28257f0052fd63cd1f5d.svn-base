...
namespace System{
	...
	class API_DB SqliteDataReader:public IDataReader
	{
	public:
		SqliteDataReader(IN void* pStmt,IN bool isBufferName);
		~SqliteDataReader();
	...
	public:  
		...
		int ColumnCount();  
		// 获取某列的名称    
		LPCTSTR GetName(IN int nCol);
		LPCSTR GetNameA(IN int nCol);

		// 获取某列的数据类型   
		FieldDataType GetDataType(IN int nCol);
