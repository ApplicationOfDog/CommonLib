...
namespace Json {
	...
	{
		//默认只支持双引号模式,这也是json内置的模式
		allowSingleQuote = false;
		allowNoneQuoteKey = false;
	}
	...
	bool Reader::readToken(Token& token) 
	{
	  ...
	  Char c = getNextChar();
	  bool ok = true,isMatchKeyword =false;//add by aweigh 20170216 isMatchKeyword
	  switch (c) 
	  {
	  case '{':
	  ...
	  case '"':
		...
		ok = readString(token.type_);	
		break;

	  case '\'':
		if(features_.allowSingleQuote)
		{
	  ...
	  default:
		if( features_.allowNoneQuoteKey && IS_LETTER(c))
		{
		  token.type_ = tokenStringNone;
		  ok = readString(token.type_);
		  break;
		}
		ok = false;
	  ...
	  if (!ok)
	  {
		  if(isMatchKeyword && features_.allowNoneQuoteKey)//add by aweigh 20170216
		  {
			  token.type_ = tokenStringNone;
			  ok = readString(token.type_);
		  }
		  if(!ok)//add by aweigh 20170216
			token.type_ = tokenError;
	  }
	  token.end_ = current_;
	  return true;
	...
	bool Reader::readString(IN TokenType type) 
	{
	  Char c = '\0';
	  while (current_ != end_) 
	  {
		c = getNextChar();
		if (c == '\\') getNextChar();
		else if(type == tokenStringNone && !IS_LETTER_OR_DIGIT(c)){
		  current_--;
		  break;
		}
	    else if (type == tokenStringSingle && c=='\'')
			break;
		else if(c == '"')
			break;
	     
	  }
	 if(type == tokenStringNone) return !IS_LETTER_OR_DIGIT(c);
	 if(type == tokenStringSingle) return c=='\'';
	 return c == '"';
	}
	...
	bool Reader::readObject(Token& tokenStart) 
	{
	  ...
	  while (readToken(tokenName)) 
	  {
		...
		name = "";
		if (tokenName.type_ == tokenStringDouble||tokenName.type_ == tokenStringSingle||tokenName.type_ == tokenStringNone) 
		{
	...
	{
		///<remark by="aweigh" date="2016/12/9">features_.allowNoneQuoteKey=true时
		/// "{x:134,y:225,w:31,h:15}"针对这样的json字符串这里会有bug
		///</remark>
		int extraCharCount = 2;//额外字符数量(即"或'字符数量)
		if(features_.allowNoneQuoteKey)//允许无引号作为key标识,可能没有额外字符
			extraCharCount = 0;

		decoded.reserve(static_cast<size_t>(token.end_ - token.start_ - extraCharCount));//为decoded预留多少空间
		Location current = NULL,end = NULL;
		if(token.type_ != tokenStringNone)
		{
			...
			else if (c == '\\') 
			{
			  ...
			  switch (escape) 
			  {
				  ...
				  case 't':
					decoded += '\t';
					break;
				  case 'u'://解析\u之后的字符,例如:"server_filename":"\u4e34\u65f6\u5185\u5bb9.txt"
					  {
						unsigned int unicode;
						if (!decodeUnicodeCodePoint(token, current, end, unicode)) return false;
						decoded += codePointToUTF8(unicode);//将该字符转为utf-8编码
					  } 
					  break;
				  default:
