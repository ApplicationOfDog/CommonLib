...
m_bLayeredChanged(false)
{
	...
	// Mark by linzw--阴影类处理
	m_ShadowInfo.bShow = false;
	m_ShadowInfo.dwColor = 0;
	m_ShadowInfo.dwDarkness = 200;
	m_ShadowInfo.dwSharpness = 5;
	m_ShadowInfo.dwSize = 3;
	m_ShadowInfo.rcCorner = CDuiRect();
	m_ShadowInfo.rcPos = CDuiRect();
	m_ShadowInfo.sImage = _T("");
	// End by linzw
}
...
void CPaintManagerUI::ReloadSkin( bool bRemoveInexistent /*= false*/ )
{
	ReloadSharedImages(bRemoveInexistent);
    for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) {
        CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
        pManager->ReloadImages(bRemoveInexistent);
    }
}
...
void CPaintManagerUI::ReloadSharedImages( bool bRemoveInexistent /*= false*/ )
{
	TImageInfo* data;
	TImageInfo* pNewData;
	CDuiPtrArray aInexistent;
	for( int i = 0; i< m_SharedResInfo.m_ImageHash.GetSize(); i++ ) {
		if(LPCTSTR bitmap = m_SharedResInfo.m_ImageHash.GetAt(i)) {
			...
			if( data != NULL ) {
				...
				if( pNewData == NULL )
				{
					if (bRemoveInexistent)
					{
						CRenderEngine::FreeImage(data, true);
						aInexistent.Add((LPVOID)bitmap);
					}
					continue;
				}
	...
	for (int i = 0; i < aInexistent.GetSize(); i++)
	{
		m_SharedResInfo.m_ImageHash.Remove((LPCTSTR)aInexistent[i]);
	}
}

void CPaintManagerUI::ReloadImages( bool bRemoveInexistent /*= false*/ )
{
	TImageInfo* data;
	TImageInfo* pNewData;
	CDuiPtrArray aInexistent;
	...
	for (int i = 0; i < aInexistent.GetSize(); i++)
	{
		m_ResInfo.m_ImageHash.Remove((LPCTSTR)aInexistent[i]);
	}
...
void CPaintManagerUI::SetWindowAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	// Mark by linzw--阴影类处理
		...
		SetShadowSharpness(_ttoi(pstrValue));
	}
	else if (_tcsicmp(pstrName, _T("shadowdarkness")) == 0) {
		SetShadowDarkness(_ttoi(pstrValue));
	}
		...
		SetShadowCorner(rcCorner);
	}
	else if (_tcsicmp(pstrName, _T("shadowimage")) == 0) {
		SetShadowImage(pstrValue);
	}
...
// Mark by linzw--阴影类处理
void CPaintManagerUI::SetShowShadow(bool bShow)
{
	m_ShadowInfo.bShow = bShow;
}

bool CPaintManagerUI::GetShowShadow()
{
	return m_ShadowInfo.bShow;
}

void CPaintManagerUI::SetShadowSize(DWORD dwSize)
{
	m_ShadowInfo.dwSize = dwSize;
}

DWORD CPaintManagerUI::GetShadowSize()
{
	return m_ShadowInfo.dwSize;
}

void CPaintManagerUI::SetShadowPos(RECT rc)
{
	m_ShadowInfo.rcPos = rc;
}

RECT CPaintManagerUI::GetShadowPos()
{
	return m_ShadowInfo.rcPos;
}

void CPaintManagerUI::SetShadowColor(COLORREF dwClolor)
{
	m_ShadowInfo.dwColor = dwClolor;
}

COLORREF CPaintManagerUI::GetShadowColor()
{
	return m_ShadowInfo.dwColor;
}

void CPaintManagerUI::SetShadowCorner(RECT rc)
{
	m_ShadowInfo.rcCorner = rc;
}

RECT CPaintManagerUI::GetShadowCorner()
{
	return m_ShadowInfo.rcCorner;
}

void CPaintManagerUI::SetShadowDarkness(DWORD dwDarkness)
{
	m_ShadowInfo.dwDarkness = dwDarkness;
}

DWORD CPaintManagerUI::GetShadowDarkness()
{
	return m_ShadowInfo.dwDarkness;
}

void CPaintManagerUI::SetShadowSharpness(DWORD dwSharpness)
{
	m_ShadowInfo.dwSharpness = dwSharpness;
}

DWORD CPaintManagerUI::GetShadowSharpness()
{
	return m_ShadowInfo.dwSharpness;
}

void CPaintManagerUI::SetShadowImage(CDuiString sImage)
{
	m_ShadowInfo.sImage = sImage;
}

CDuiString CPaintManagerUI::GetShadowImage()
{
	return m_ShadowInfo.sImage;
}
// End by linzw
} // namespace DuiLib
