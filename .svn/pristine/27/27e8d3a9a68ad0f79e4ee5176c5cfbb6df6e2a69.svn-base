...
// Mark by linzw-- 阴影类处理
// Structure for Shadow Class
typedef struct tagTShadowUI
{
	bool bShow;
	CDuiString sImage;
	DWORD dwSize;
...
public:
    ...
    static void ReloadSkin(bool bRemoveInexistent = false);
	...
	static void ReloadSharedImages(bool bRemoveInexistent = false);
	void ReloadImages(bool bRemoveInexistent = false);
	...
	bool GetShowShadow();
	void SetShadowSize(DWORD dwSize);
	DWORD GetShadowSize();
	void SetShadowPos(RECT rc);
	RECT GetShadowPos();
	void SetShadowColor(COLORREF dwClolor);
	COLORREF GetShadowColor();
	void SetShadowCorner(RECT rc);
	RECT GetShadowCorner();
	void SetShadowDarkness(DWORD dwDarkness);
	DWORD GetShadowDarkness();
	void SetShadowSharpness(DWORD dwSharpness);
	DWORD GetShadowSharpness();
	void SetShadowImage(CDuiString sImage);
	CDuiString GetShadowImage();
	// End by linzw
private:
