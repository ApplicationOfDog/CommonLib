...
void CControlUI::SetAttributeList(LPCTSTR pstrList)
{
    ...
    while( *pstrList != _T('\0') ) {
		...
		TCHAR cQuot = *pstrList++;
        while( *pstrList != _T('\0') && *pstrList != cQuot ) {
            LPTSTR pstrTemp = ::CharNext(pstrList);
            while( pstrList < pstrTemp) {
                sValue += *pstrList++;
            }
        ...
        ASSERT( *pstrList == cQuot );
        if( *pstrList++ != cQuot ) return;
...
{
	bool bBorderNotNull = m_rcBorderSize.left > 0 ||
		m_rcBorderSize.right > 0 ||
		m_rcBorderSize.top > 0 ||
		m_rcBorderSize.bottom > 0;

	if (bBorderNotNull && (m_dwBorderColor != 0 || m_dwFocusBorderColor != 0)) {
		if( m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0 )//画圆角边框
		{
			...
			if (m_rcBorderSize.right == m_rcBorderSize.left && m_rcBorderSize.top == m_rcBorderSize.left && m_rcBorderSize.bottom == m_rcBorderSize.left) {
				...
				if(m_rcBorderSize.right > 0) {
					rcBorder		= m_rcItem;
					rcBorder.left = m_rcItem.right - (m_rcBorderSize.right + 1) / 2;
                    rcBorder.right  = rcBorder.left;
					if (IsFocused() && m_dwFocusBorderColor != 0)
				...
				if(m_rcBorderSize.bottom > 0) {
					rcBorder		= m_rcItem;
					rcBorder.top = m_rcItem.bottom - (m_rcBorderSize.bottom + 1) / 2;
                    rcBorder.bottom = rcBorder.top;
                    rcBorder.left  += m_rcBorderSize.left;
                    rcBorder.right -= m_rcBorderSize.right;
					if (IsFocused() && m_dwFocusBorderColor != 0)
