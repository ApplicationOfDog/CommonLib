...
void String::replace(const std::string & before, const std::string & after, bool caseSensitive) {
	...
	while ((pos = tmp.find(before2, pos)) != string::npos) {
		...
		str_.replace(pos, before2.length(), after);
...
std::string String::trim() {
	...
	/**/
	string::size_type pos1 = str_.find_first_not_of(" \t");
	string::size_type pos2 = str_.find_last_not_of(" \t");	
	newstr = str_.substr(pos1 == string::npos ? 0 : pos1,
	//*/
	/*/
